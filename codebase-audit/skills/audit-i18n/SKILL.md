...
2. **Filter**: read `${CLAUDE_PLUGIN_ROOT}/shared/detection/07-i18n.md`. Drop any checklist item whose
...
3. **Detect (two-stage, spec §5a.2 / §9a.4)**:
   ...
   b. Read only candidate snippets; apply each item's `confirm` rule. Assign severity
      honoring any severity floors. Consult `${CLAUDE_PLUGIN_ROOT}/shared/detection/examples/07-i18n.md` only
