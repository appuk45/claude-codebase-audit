---
name: audit-issues
description: Maintainability/issue-detection dimension audit — emits AuditResult JSON; runs standalone or as an orchestrator subagent.
---

# /audit-issues

Audit the current working directory for the Issue Detection dimension.
...
2. **Filter**: read `${CLAUDE_PLUGIN_ROOT}/shared/detection/04-issues.md`. Drop any checklist item whose
...
3. **Detect (two-stage, spec §5a.2 / §9a.4)**:
   ...
   b. Read only candidate snippets; apply each item's `confirm` rule. Assign severity
      honoring any severity floors. Consult `${CLAUDE_PLUGIN_ROOT}/shared/detection/examples/04-issues.md` only
...
4. **Emit**: produce one `AuditResult` JSON object conforming to `${CLAUDE_PLUGIN_ROOT}/shared/schema.json` with
   `audit: "issues"`, `iso_characteristic: "Maintainability"`, the checklist (with
