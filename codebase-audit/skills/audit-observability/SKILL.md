...
description: Observability dimension audit (OWASP A09:2025) — emits AuditResult JSON; runs standalone or as an orchestrator subagent.
---

# /audit-observability

Audit the current working directory for the Observability dimension.
...
2. **Filter**: read `${CLAUDE_PLUGIN_ROOT}/shared/detection/11-observability.md`. Drop any checklist item whose
...
3. **Detect (two-stage, spec §5a.2 / §9a.4)**:
   ...
   b. Read only candidate snippets; apply each item's `confirm` rule. Assign severity
      honoring any severity floors. Consult `${CLAUDE_PLUGIN_ROOT}/shared/detection/examples/11-observability.md` only
...
4. **Emit**: produce one `AuditResult` JSON object conforming to `${CLAUDE_PLUGIN_ROOT}/shared/schema.json` with
   `audit: "observability"`, `iso_characteristic: "Operability"`, the checklist (with
