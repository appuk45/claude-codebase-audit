...
description: Resilience & fault-tolerance dimension audit — emits AuditResult JSON; runs standalone or as an orchestrator subagent.
---

# /audit-resilience

Audit the current working directory for the Resilience & Fault Tolerance dimension.
...
2. **Filter**: read `${CLAUDE_PLUGIN_ROOT}/shared/detection/09-resilience.md`. Drop any checklist item whose
...
3. **Detect (two-stage, spec §5a.2 / §9a.4)**:
   ...
   b. Read only candidate snippets; apply each item's `confirm` rule. Assign severity
      honoring any severity floors. Consult `${CLAUDE_PLUGIN_ROOT}/shared/detection/examples/09-resilience.md` only
...
4. **Emit**: produce one `AuditResult` JSON object conforming to `${CLAUDE_PLUGIN_ROOT}/shared/schema.json` with
   `audit: "resilience"`, `iso_characteristic: "Fault Tolerance"`, the checklist (with
