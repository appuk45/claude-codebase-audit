...
description: Performance dimension audit (ISO 25010 Performance Efficiency) — emits AuditResult JSON; runs standalone or as an orchestrator subagent.
---

# /audit-performance

Audit the current working directory for the Performance dimension.
...
2. **Filter**: read `${CLAUDE_PLUGIN_ROOT}/shared/detection/01-performance.md`. Drop any checklist item whose
...
3. **Detect (two-stage, spec §5a.2 / §9a.4)**:
   ...
   b. Read only candidate snippets; apply each item's `confirm` rule. Assign severity
      honoring any severity floors. Consult `${CLAUDE_PLUGIN_ROOT}/shared/detection/examples/01-performance.md` only
...
4. **Emit**: produce one `AuditResult` JSON object conforming to `${CLAUDE_PLUGIN_ROOT}/shared/schema.json` with
   `audit: "performance"`, `iso_characteristic: "Performance Efficiency"`, the checklist (with
